const clientId = "3a588af26b194c39954b657368420097";
...
const Spotify = {
  getAccessToken() {
    ...
    if (accessToken & accessTokenMatch) {
      ...
      window.history.pushState('Access Token', null, '/'); // Tthis clears the parameter, allowing us to get a new access token when it expires
      return accessToken;
    } else {
  ...
  search(term) {
    ...
    return fetch(`http://api/spotify.com/v1/search?type=track&g=${term})`, {
      headers: {
